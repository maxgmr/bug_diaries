...
```shell
autopkgtest-buildvm-ubuntu-cloud -s 128G --ram-size=8192 --cpus=4 -v -r questing
```
...
First, we must edit `debian/tests/control`:

```diff
-Test-Command: ./debian/rules build RUST_TEST_SELFBUILD=1
+Test-Command: psrecord "./debian/rules build RUST_TEST_SELFBUILD=1"
 Depends: @, @builddeps@
 Restrictions: rw-build-tree, allow-stderr
```

Then, we can run this command on both the `rustc-1.84` and `rustc-1.85` Git branches:
...
Unfortunately, this essentially _doubles_ the build time because the modified source package must be built into binary packages before starting the autopkgtests.
