...
If the previous Rust toolchain's autopkgtests fail for the _same reason_, and increasing the image size and VM memory causes the tests to pass, then it seems like we simply need to allocate more resources to the autopkgtest runner.

If this is true, then there remains one question: _how do we allocate more resources to the autopkgtest runner_?
